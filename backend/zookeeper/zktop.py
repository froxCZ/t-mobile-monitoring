...
if sys.platform == 'win32':
  print("Cannot run natively on Windows due to missing curses package. Try running in a cygwin shell instead!");
  sys.exit(1)
import curses
...
def get_zk_servers(filename):
  if filename:
    ...
    return ','.join(
      "%s:%s" % (s.strip(), ZK_DEFAULT_PORT) if not ':' in s else "%s" % s for s in options.servers.split(',', 1))


if __name__ == '__main__':
  for i in range(0,5):
    x = send_cmd("localhost", 2181+i, b'stat\n')
    print(x)