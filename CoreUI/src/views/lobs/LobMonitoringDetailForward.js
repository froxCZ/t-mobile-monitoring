...
export default class LobMonitoringDetailForward extends Component {
  ...
  propChange(props) {
    ...
    let forwardName = props.params.forwardName;
    if (this.state.lobName != lobName || this.state.forwardName != forwardName) {
      this.setState({lobName: lobName, forwardName: forwardName});
  ...
  loadData(fromDate, toDate, granularity) {
    Api.lobData(fromDate, toDate, this.state.lobName, [],[this.state.forwardName], granularity)
      .then(response => {
      ...
      <div className="animated fadeIn">
        <div className="row">
          <div className="col-lg-12">
            <h2>{this.state.lobName} - {this.state.forwardName}</h2>
          </div>
        </div>
        <div className="row">
